...
for country in ['India']:
    try:
        ...
        plt.plot(list(range(xlim))[1:], pred, color='black', label='Robust Weibull Prediction (dead)')
        _ = plt.bar(x[:training_data], data[:training_data], width=1, color='blue',edgecolor='black', linewidth=0.01, alpha=0.2, label='Train Data (dead)')
        _ = plt.bar(x[training_data:], data[training_data:], width=1, color='red',edgecolor='black', linewidth=0.01, alpha=0.2,label='Test Data (dead)')
        plt.ylabel("Number of deaths"); plt.xlabel("Date"); plt.tight_layout()
